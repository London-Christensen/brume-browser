...
;
; 1. Welcome Page - REMOVED FOR BRUME
;    Brume ships a deliberately minimal installer: choose a location, set
;    options, install. A welcome page carries no information and no decision,
;    so it is exactly the screen that requirement exists to remove.
;
...
;
; One decision only: whether Brume checks for updates on launch. The value is
; written to the registry by Section Install and read by the app at startup,
; which is how a choice made in the installer reaches the running program.
;
; Two behaviours here are easy to get wrong and expensive to debug later:
;
;   - The updater relaunches this same installer with /P /UPDATE, so this page
;     never renders during an update. $AutoUpdateEnabled must therefore already
;     hold the right value before the page would have run - see .onInit.
;
;   - An update must never silently re-enable checks the user turned off. That
...
Function PageOptions
  ; Passive and silent runs keep whatever .onInit resolved.
  ${If} $PassiveMode = 1
    Abort
  ${EndIf}

  !insertmacro MUI_HEADER_TEXT "Options" "Choose how $(^Name) keeps itself up to date."
...
Function PageOptionsLeave
  ${NSD_GetState} $AutoUpdateCheckbox $AutoUpdateEnabled
FunctionEnd
...
Function .onInit
  ...
  ; Seed the auto-update preference - BRUME ADDITION
  ;
...
Section Install
  ...
  ; Persist the auto-update preference - BRUME ADDITION
  ;
  ; This is the handoff from installer to application. The app reads this DWORD
  ; on first launch to decide whether to check for updates, then owns the
  ; setting from that point on via its own settings panel.
  ;
  ; $AutoUpdateEnabled is either what the user ticked on the Options page, or -
  ; when that page was skipped for a passive/update install - the value .onInit
  ; read back from this very key, so rewriting it here is a no-op rather than a
  ; reset.
  WriteRegDWORD SHCTX "${MANUPRODUCTKEY}" "AutoUpdate" $AutoUpdateEnabled
